...
class TradingAgent:
    ...
    async def run(self, task: str = None, date: str = None, stocks: list[str] = None, blacklist: list[str] = None):
...
- Nếu việc gọi công cụ thất bại, hãy cố gắng tìm kiếm thông tin tương ứng từ Internet.{f"\n- Loại bỏ các mã cổ phiếu liên quan đến lĩnh vực hoặc nhóm: {', '.join(blacklist)}." if blacklist is not None else ""}
...
- Các mã cổ phiếu đang nắm giữ, theo định dạng <Mã cổ phiếu> (thị giá mua vào trung bình): {"" if stocks is None else ', '.join(stocks)}
