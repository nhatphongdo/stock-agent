...
def init_db():
    ...
    if "dividend_rate" not in columns:
        cursor.execute("ALTER TABLE users ADD COLUMN dividend_rate REAL DEFAULT 0.0")
        print("✅ Migration: Added dividend_rate column to users table")
    if "profit_rate" not in columns:
        cursor.execute("ALTER TABLE users ADD COLUMN profit_rate REAL DEFAULT 0.0")
        print("✅ Migration: Added profit_rate column to users table")
...
def update_user_settings(
    ...
    """Update user settings including whitelist, dividend_rate, and profit_rate."""
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    updates = ["black_list = ?", "return_rate = ?"]
    params = [json.dumps(black_list), return_rate]

    if white_list is not None:
        updates.append("white_list = ?")
        params.append(json.dumps(white_list))
    if dividend_rate is not None:
        updates.append("dividend_rate = ?")
        params.append(dividend_rate)
    if profit_rate is not None:
        updates.append("profit_rate = ?")
        params.append(profit_rate)
